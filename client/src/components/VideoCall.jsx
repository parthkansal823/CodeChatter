...
export default function VideoCall({ onBack, roomName, collaborators = [] }) {
  const { user } = useAuth();
  ...
  // Call duration timer — starts once permissions granted
  useEffect(() => {
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        {hasPermissions === false ? (
          ...
          >
            ...
            {/* Avatar fallback when video muted */}
            ...
            {/* PiP overlay button — shows on hover */}
        ...
        {remoteCollaborators.map((c, i) => (
          ...
          >
            <div className="flex flex-col items-center gap-2">
              <UserAvatar username={c.username} size="lg" className="h-16 w-16 shadow-xl" />
              <span className="text-xs text-zinc-400">Not in call yet</span>
            </div>
            <div className="absolute bottom-2 left-2 flex items-center gap-1.5 rounded-lg border border-white/10 bg-black/50 px-2 py-1 backdrop-blur-md">
              <span className="text-[10px] font-medium text-white">{c.username}</span>
            </div>
          </div>
        ))}
